...
def _backend_base_url() -> str:
    return os.getenv("PAYLY_BACKEND_BASE_URL", "http://127.0.0.1:8000")


def _redirect_uri(provider: str) -> str:
    return f"{_backend_base_url()}/integrations/{provider}/oauth/callback"
...
def oauth_start(
    ...
    _STATE_STORE.put(state=state, user_id=current_user.id, provider=provider, code_verifier=code_verifier)
